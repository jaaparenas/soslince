      ...
      <v-table height="600px" fixed-header style="min-height: 600px;">
        <thead>
          <tr>
            ...
            <th class="text-center">{{ $t('commons.common.attended') }}</th>
            <th></th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="item in listSOS?.results" :key="item.id">
...
const loadData = async () => {
  listSOS.value = await uCustomer.getListSos(props.userId, page.value, (rangeFixed as any).value);
  numPages.value = Math.ceil(listSOS.value.count / 10);
};
...
const clickExport = async () => {
  uCustomer.getExport('sos', props.userId, (rangeFixed as any).value);
}
